...
subroutine ann_check_symmetry_function(parini)
    ...
    integer:: iat,jat,i,j,ig,jg,iconf,jconf, ng, nat,a,i0,iati,jatj
...
!-----------------Compute symmetry functions with/without normalization-------------------------
    ...
    !----------------------------------------------------------   
        ...
        if(parini%plot_descriptor_distance) then
            fnout='fingerprint_grid.plot'
            open(unit=13,file=fnout,status='replace',iostat=ios)
            if(ios/=0) then
                write(*,'(a)') 'ERROR: failure openning output file '//fnout
                stop
            endif
    ...
    if (.not. parini%atoms_descriptor_distance) then !Global structural distance
        do iconf=1,atoms_check%nconf
            ...
            do jconf=1,atoms_check%nconf
                    jatj = jatj + 1
                    if (iati.lt.jatj) then
                        do iat=1,atoms_check%atoms(iconf)%nat
                        ...
                        if(parini%plot_descriptor_distance) then
                            write(13,'(i6,1x,i6,1x,es9.2)') iati,jatj,distance
                        endif
                        ...
                        if(parini%plot_descriptor_distance) then
                            if (iati.ne.jatj) then
                                write(13,'(i6,1x,i6,1x,es9.2)') iati,jatj,-1.d0
                            else
                                write(13,'(i6,1x,i6,1x,es9.2)') iati,jatj, 0.d0
                            endif
                        endif
                    endif
            enddo !over jconf
            if(parini%plot_descriptor_distance) write(13,*) " "
        deallocate(diff,c,F) 
        enddo !over iconf
    ...
    !Here we write the plot script for gnuplot
        ...
        write(29,'(a,i5,a)') "set xrange[0:",iati+1,"]"
        write(29,'(a,i5,a)') "set yrange[0:",jatj+1,"]"
